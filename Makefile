...
GCCFLAGS=-march=rv32imc -mabi=ilp32 -mcmodel=medany -O3
OBJFLAGS=--disassemble-all --disassemble-zeroes 
# --section=.text --section=.text.startup --section=.text.init --section=.data
LINK_FLAGS = -march=rv32imc -mabi=ilp32 -static -nostdlib -nostartfiles -T $(CORE)/link.ld
...
build : clean build-drivers
	...
	$(GCC) $(GCCFLAGS) -I$(INCLUDE) -I$(LIBS) -I$(DRIVERS)/timer -I$(DRIVERS)/plic -I$(DRIVERS)/pwm -I$(DRIVERS)/gpio -c $(FILEPATH)/$(PROGRAM).c -o $(FILEPATH)/output/$(PROGRAM).o -lgcc
...
test : clean 
	...
	@mkdir $(FILEPATH)/output
	@mkdir -p generated
	$(GCC) $(GCCFLAGS) -I $(INCLUDE) -c $(DRIVERS)/timer/timer.c -c $(DRIVERS)/sa/uart.c $(DRIVERS)/spi/spi.c $(DRIVERS)/pwm/pwm.c -o generated/timer.o generated/spi.o generated/uart.o generated/pwm.o -lgcc
